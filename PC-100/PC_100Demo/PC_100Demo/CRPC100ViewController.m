...
@interface CRPC100ViewController ()<CRBlueToothManagerDelegate, CRPC_300SDKDelegate>
...
/** 固件升级*/
@property (strong, nonatomic) NSString *currentHardVersion;
@property (strong, nonatomic) NSString *currentSoftVersion;
@property (strong, nonatomic) NSData *iapData;

@end

@implementation CRPC100ViewController
...
- (void)bleManager:(CRBlueToothManager *)manager didSearchCompleteWithResult:(NSArray<CRBleDevice *> *)deviceList {
    ...
    for (CRBleDevice *device in deviceList) {
        if ([device.bleName containsString:pc100] || [device.bleName containsString:pc200]) {
...
#pragma mark - —— 固件升级 ——
- (IBAction)updateIAP:(UIButton *)sender {
    /** 查询固件版本信息。参数mode(1:准备升级固件；2:只回应版本信息)*/
    [[CRPC_300SDK shareInstance] queryDeviceIAPVersionWithMode:1 ForDevice:self.device];
}

#pragma mark - 获取固件版本信息
- (void)pc_300SDK:(CRPC_300SDK *)pc_300SDK GetIAPState:(int)state HardWareVersion:(NSString *)hwVersion SoftWareVersion:(NSString *)swVersion ForDevice:(CRBleDevice *)device {
    //保存当前设备的版本信息
    self.currentHardVersion = hwVersion;
    self.currentSoftVersion = swVersion;
    //获取服务器上的最新版本信息，并比较软件版本
    if (state == 1)
        [self getLastIAPInfoForServer];
}

#pragma mark - 获取服务器最新的固件信息
- (void)getLastIAPInfoForServer {
    __weak typeof(self) weakSelf = self;
    [[CRHttpRequest shareInstance] getPC200IAPInfoWithCompleteHandle:^(BOOL success, NSError *error, NSDictionary *dict) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                //比较版本
                int state = [weakSelf checkPossibilityOfUpdatingWithIAPInfo:dict];
                if (state == 0) {
                    //可以升级
                    [weakSelf displayIPAUpdateAlertWithIAPInfo:dict];
                } else {
                    NSArray *array = @[@"", @"已经是最新版本", @"软件版本过低，不支持升级", @"硬件版本过低，不支持升级"];
                    NSString *info = array[state];
                    [weakSelf displayQueryResultsWithString:info];
                }
            } else {
...
/** 提示并确认升级固件*/
- (void)displayIPAUpdateAlertWithIAPInfo:(NSDictionary *)iapInfo {
    //版本升级描述
    NSString *updateInfo = iapInfo[@"updateInfo"];
    NSString *info = [NSString stringWithUTF8String:[updateInfo cStringUsingEncoding:NSUTF8StringEncoding]];
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"发现新版固件可升级" message:info preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"升级" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        //确认升级固件
        NSString *fileUrl = iapInfo[@"fileUrl"];
        [self downloadIAPDataWithURL:fileUrl];
    }];
...
/** 进行版本比较  0:可以升级 1:已经是最新版本  2.软件版本过低  3.硬件版本过低 */
- (int)checkPossibilityOfUpdatingWithIAPInfo:(NSDictionary *)iapInfo {
    //最新版本
    //NSString *hardwareV = ipaInfo[@"hardwareV"];
    NSString *softwareV = iapInfo[@"softwareV"];
    //允许升级的最低版本
    NSString *lowhardwareV = iapInfo[@"lowHardwareV"];
    NSString *lowsoftwareV = iapInfo[@"lowSoftV"];
    //当前设备版本
    int currentHardV = [self.currentHardVersion stringByReplacingOccurrencesOfString:@"." withString:@""].intValue;
    int currentSoftV = [self.currentSoftVersion stringByReplacingOccurrencesOfString:@"." withString:@""].intValue;
    //默认可以升级
    int state = 0;
    //已经是最新版本
    if (currentSoftV >= softwareV.intValue)
        state = 1;
    //软件版本过低，不支持升级
    if (currentSoftV < lowsoftwareV.intValue)
        return 2;
    //硬件版本过低，不支持升级
    if (currentHardV < lowhardwareV.intValue)
        return 3;
    return state;
}

#pragma mark - 从服务器下载最新的固件包
- (void)downloadIAPDataWithURL:(NSString *)urlString {
    [CRProgress showProgressWithTitle:@"正在下载固件，请稍等..." InView:self.view];
    __weak typeof(self) weakSelf = self;
    [[CRHttpRequest shareInstance] getPC200IAPDataPackageWithUrl:urlString CompleteHandle:^(BOOL success, NSError * _Nonnull error, NSData * _Nonnull data) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                [CRProgress showProgressWithTitle:@"下载完成！开始更新..." InView:self.view];
                //保存数据
                self.iapData = data;
                //开启固件更新
                [weakSelf startIAPUpdate];
            } else {
                NSString *string = [NSString stringWithFormat:@"%@", error.localizedDescription];
                [self displayQueryResultsWithString:string];
            }
...
#pragma mark - 开启固件更新
- (void)startIAPUpdate {
    [[CRPC_300SDK shareInstance] startIAPUpdateForDevice:self.device];
}

- (void)pc_300SDK:(CRPC_300SDK *)pc_300SDK GetUpdateIAPResponceForDevice:(CRBleDevice *)device {
    //开启固件升级模式后，开始传输固件数据
    [self startTransmistIAPData:self.iapData];
}

#pragma mark - 开始发送数据
- (void)startTransmistIAPData:(NSData *)ipaData {
    [[CRPC_300SDK shareInstance] startTransmistIAPData:ipaData ForDevice :self.device];
}
...
- (void)pc_300SDK:(CRPC_300SDK *)pc_300SDK GetIAPUpdateProgress:(float)progress ForDevice:(CRBleDevice *)device {
    ...
    [CRProgress showProgressWithTitle:string InView:self.view];
    NSLog(@"%@", string);
}

#pragma mark - 完成固件更新（更新完自动回调）
- (void)pc_300SDK:(CRPC_300SDK *)pc_300SDK IAPUpdateCompleteWithState:(int)state ForDevice:(CRBleDevice *)device {
    [CRProgress hiddenProgress];
    [self displayQueryResultsWithString:@"固件升级完成！"];
}
